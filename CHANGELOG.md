...
- **CI quality gates** — GitHub Actions unit-tests job now runs `npm audit --audit-level=high` (blocks high-severity CVEs), `npm run typecheck` (blocks type regressions), `npm run lint` (blocks linting regressions), and `npm run format:check` (blocks formatting drift) before the test step — closing the gap where the local `npm run validate` checked types and formatting but CI did not.
