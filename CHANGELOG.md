...
- **Self-healing E2E flakiness — AI healing responses are now deterministic.** The Gemini and OpenAI healing calls ran at each provider's default sampling temperature (~1.0) with no system instruction, so an identical prompt intermittently returned a clean selector and intermittently a chain-of-thought essay (`* Task: Find a replacement selector…`) that `parseAIResponse()` could not reduce to a usable selector — surfacing as `flaky` self-healing tests across Chromium and WebKit shards. Both providers now run at `temperature: 0` and carry a dedicated system instruction constraining the reply to a single bare selector (or `FAIL`). `parseAIResponse()` was additionally hardened to salvage the selector from a multi-line/bulleted reply as defence-in-depth (it now reduces a verbose block to the last selector-like line).
